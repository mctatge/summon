...
// a session, and CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1 keeps the CLI from refetching flags, rewriting ~/.claude.json
// and rotating its backup ring on every read (both measured 2026-09-19; the plan and windows are still reported).
...
export async function readClaudeUsage({executable=findExecutable,spawnChild=spawnLongLived,timeoutMs=15_000,now=()=>Date.now()}={}){
  ...
  const outcome=await exchange({binary,args:[...CLAUDE_USAGE_ARGS],env:scrubbedEnv({CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC:'1'}),spawnChild,timeoutMs,
  ...
  // rate_limits:null with the flag true is the CLI saying its own usage fetch failed (expired token, 5xx): a read failure, not 0 %.
  if(!isObject(body.rate_limits))return done('error',{plan,error:'Claude could not fetch its limits; its usage endpoint did not answer.'});
