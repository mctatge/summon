...
export function groupingArgs(engine,{schemaPath,schemaJson,effort='medium',claudeModel='opus',systemPrompt=SYSTEM_PROMPT}={}){
...
export async function runGrouping(engine,{prompt,schema,effort='medium',claudeModel='opus',systemPrompt}={},{executable=defaultExecutable,run=defaultRun,scrubbedEnv=defaultScrubbedEnv,tmp=tmpdir()}={}){
  checked(engine,effort,claudeModel);
  ...
  try{
    ...
    const args=groupingArgs(engine,{schemaPath,schemaJson,effort,claudeModel,systemPrompt});
