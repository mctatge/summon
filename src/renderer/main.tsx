...
function Preferences({ snapshot, busy, onClose, onChange, onLink, onChooseModel, onShowWidget, setVoiceMode, voiceActive, error, onEnroll, onInstallHooks, hookStatus, onRefreshUsage, onUsageSettings }: { snapshot: Snapshot; busy: boolean; onClose: () => void; onChange: (patch: SettingsPatch) => Promise<boolean>; onLink: (name: 'calendar' | 'benchmark' | 'accessibility' | 'microphone' | 'input-monitoring' | 'data-folder' | 'file-access') => Promise<unknown>; onChooseModel: () => Promise<unknown>; onShowWidget: () => Promise<unknown>; error: string; setVoiceMode: (mode: VoiceMode) => void; voiceActive: boolean; onEnroll: () => void; onInstallHooks?: () => Promise<{ installed: boolean; backup: string | null; events: string[] }>; hookStatus?: () => Promise<ClaudeHooksStatus>; onRefreshUsage?: () => Promise<unknown>; onUsageSettings?: (patch: Partial<UsageSettings>) => Promise<boolean> }) {
