/*!
 * Agenttrail Kitchen — Copyright (c) 2026 Kelly Sun
 * SPDX-License-Identifier: MIT
 * Source: sodiumsun/agenttrail@e4ba2daa270abd7ecb51565bbfb12386f47ea418
...
// Summon lifecycle addition: call only after all host scenes stop rendering and
// release their owned clones. Never reset while a scene uses shared originals.
export function resetArtCaches(){
  for(const geometry of cache.values())geometry.dispose();
  for(const mat of materials.values())mat.dispose();
  for(const mat of [woodMat,tileMat,groundMat,shadowMat]){
    if(mat){mat.map?.dispose();mat.dispose();}
  }
  cache.clear();materials.clear();
  woodMat=tileMat=groundMat=shadowMat=undefined;
  seed=24871;
}
