...
try{
  ...
  await page.waitForFunction(async()=>(await window.summon.snapshot()).settings.accentColor==='#ffffff');
  const contrast=await page.evaluate(()=>({text:getComputedStyle(document.documentElement).getPropertyValue('--on-accent').trim(),meter:getComputedStyle(document.documentElement).getPropertyValue('--accent-meter').trim()}));
  assert.notEqual(contrast.text,'#ffffff','White custom accent must retain dark labels');
  ...
  await page.getByRole('dialog',{name:'Start a new task',exact:true}).waitFor();
  await page.getByRole('button',{name:'Cancel',exact:true}).click();
  assert.equal(await page.getByRole('dialog').count(),0);
  // The real dialog and preload run, but launching another app is isolated.
  await electron.evaluate(({ipcMain})=>{
    globalThis.__launchUISmoke=[];
    ipcMain.removeHandler('summon:agent-launch');
    ipcMain.handle('summon:agent-launch',(_event,options)=>{
      globalThis.__launchUISmoke.push(options);
      if(globalThis.__launchUISmoke.length===1)throw new Error('Synthetic launch failure');
      return {app:options.app,hooks:true,modelSelection:{reason:'Synthetic model selection.',source:'https://aistupidlevel.info/'}};
    });
  });
  await page.getByRole('button',{name:'New task',exact:true}).click();
  await page.getByRole('combobox',{name:'Workspace',exact:true}).selectOption({label:'Harbor'});
  await page.getByRole('button',{name:'Start Codex',exact:false}).click();
  await page.getByRole('alert').filter({hasText:'Synthetic launch failure'}).waitFor();
  assert.equal(await page.getByRole('combobox',{name:'Workspace',exact:true}).inputValue(),(await page.evaluate(()=>window.summon.snapshot())).projects.find(project=>project.name==='Harbor').id,'Failed launches preserve the selected workspace');
  await page.getByRole('button',{name:'Start Codex',exact:false}).click();
  await page.getByText('Codex opened in Terminal for Harbor.',{exact:false}).waitFor();
  const launchCalls=await electron.evaluate(()=>globalThis.__launchUISmoke);
  assert.equal(launchCalls.length,2);
  assert.equal(launchCalls[1].app,'codex');
  ...
  await commandBox.fill('find my Excel file');await commandBox.press('Enter');
  ...
  assert.deepEqual(errors,[]);console.log('Electron UI smoke passed: overview, persisted/custom accents, picker Escape, task launch retry/payload, bridge, files, routine save/alias, memory save/search/forget, preferences, pause, invalid IDs, compact layouts.');
