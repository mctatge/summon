...
- Calendar access and AI Stupid Level rankings from its public feed or optional data API, with a local one-hour cache. New Claude sessions select the highest-ranked eligible model available through the installed CLI.
...
- **Usage**: how much of your Claude and Codex plans is used, as each CLI reports about itself through one bounded, tool-less call (no prompt, no turn, no token read), checked every five minutes, shown in the menu bar and Preferences, and used by **Ask Auto** and the `pick_engine` MCP tool to support local task routing with quota and your answer ratings. See [Usage](docs/desktop-companion.md#usage) and [Task routing](docs/desktop-companion.md#task-routing).
- **Visual workspace**: connected views of local commit history, observed code imports, explicit goals and milestones, an animated 3D agent kitchen across projects, and reported hook events. The kitchen opens on the project selected in Working in, or All projects when none is selected; project filters remain available. Open **Visual workspace** in the title bar or press **⌘⇧V** outside a text field. Goals can be inferred from evolving conversations, app context and project work, with visible evidence and a choice of local, Claude or Codex reasoning; saved goals remain yours to edit. Event coverage depends on each provider's hooks. See [Visual workspace](docs/desktop-companion.md#visual-workspace).
- **Durable work records**: saved repository goals keep a checklist, acceptance criteria, findings and evidence, a next step, session history and coordination constraints across sessions. Connected agents read and checkpoint the same records through MCP; an agent's completion report remains **Needs verification** until you confirm it. See [Durable work records](docs/desktop-companion.md#durable-work-records).

Optional **Read screen text locally** uses in-memory images of the selected app’s front window during explicit desktop tasks. Images stay on the Mac; extracted text follows the visible Local/Codex/Claude reasoning choice. There is no background screen recording, raw keystroke capture, arbitrary document-content indexing, or browser-history monitoring. See [scope, setup, and limitations](docs/desktop-companion.md).
